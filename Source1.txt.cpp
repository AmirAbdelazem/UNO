...
public:
    ...
    int Number_of_players;//must deal with it in all functions depend on  number of players
    ...
    UNO()
    {
        ...
        Number_of_players = MAX_NUM_OF_PLAYERS;
    }

    Player UNO_players[MAX_NUM_OF_PLAYERS];// array of max players 
    void Create_player(Queue*);//create players and give them 7 cards
    void GET_Number_of_players();//creat 'n' playersn--> number of players
    ...
    void display_player_card();
    bool Game_finished();
};
//creat 'n' playersn--> number of players 
void UNO::GET_Number_of_players()//creat 'n' players n--> number of players
{
    int num;
    ...
    if (num == 2 || num == 3 || num == 4 || num == 5 || num == 6 || num == 7 || num == 8 || num == 9 || num == 10)
        ...
        cout << " \a\a please enter availd number between 2 and 10 ";
        GET_Number_of_players();
    }
}
//create players and give them 7 cards
void  UNO::Create_player(Queue* pulling) {
...
void  Player::Set_color_of_ground(card* temp)
{
    card flag;
    cout << "\n Please Set the ground color by enter first char\n ";
    cout << "'R'=>Red\t'G'=>Green\t'B'=>Blue\t'Y'=>Yellow  \n ";
    cin >> flag.Color;
    if (flag.Color == 'R' || flag.Color == 'r' || flag.Color == 'G' || flag.Color == 'g' || flag.Color == 'B' || flag.Color == 'b' || flag.Color == 'Y' || flag.Color == 'y')
    {
...
{
    card temp;//to make the check on special cards
    temp = pulling->dequeue(pulling);
    switch (temp.value)
    {
        // 10-->WILD CARD
    case 10: {
        card* temp2;//virtual card contain color of wild
        UNO_Ground->Ground_Cards[UNO_Ground->top] = temp;
        UNO_Ground->top++;
        counter++;
        UNO_players[counter].Set_color_of_ground(temp2);//player 0 set color
        card check;
        counter++;
        check = UNO_players[counter].UNO_Color.Get_card_by_color(*temp2);
        UNO_players[counter].Play_by_color_due_to_wild(check, pulling, UNO_Ground, *temp2);//if pulled make colliosn
    ...
    case 12: {
        ...
        counter++;
        UNO_Ground->Ground_Cards[0] = temp;
        UNO_Ground->top++;
        break;
    }//13-->Reverse  depened on color
    case 13: {

        REVERS_CARD_set_flag(temp);
        UNO_Ground->Ground_Cards[0] = temp;
        UNO_Ground->top++;
...
void UNO::player_card_into_Ground(Queue* pulling, Ground* UNO_Ground)
{
    ...
    top_ground_card = UNO_Ground->Ground_Cards[UNO_Ground->top - 1];//get the card of top of ground
...
int main()
{
    ...
    Ground UNO_GND;
    UNO amir;
    amir.GET_Number_of_players();

    amir.Create_player(&PULLING_CARDS);
...
/*
* UNO OBJ;
 obj.counter=-1;//initial
*
*obj.flag_rev=false;//start false
*
* get first card into ground();//change in flag if it was card.value==13//before the loop of the game
 while(gamenotfinised//function)
 {
          if(obj.flag_rev)
          {
